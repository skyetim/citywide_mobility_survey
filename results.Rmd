...
For females, both prefer and not prefer bicycling, the majority of them think bicycling is inexpensive or / and convenient, although the differences between the proportions of the supporters are a little large. The proportion of females that think riding a bicycle is safe enough is the least, expecially for those prefering bicycling. This indicates that the females choosing bicycle as the mode to get around city might value low cost and high level of convenience most when comparing safety with these two advantages. This is the same for males. Now consider the result showing in figure "Gender Composition" that almost 70% people who prefer bicycling are males. We can conclude that more males would pay more attention to the advantages of bicycle (e.g. inexpensiveness, convenience, etc.) than females. In other words, more females may be concerned about safety most, which is consistent with our guess.
...
ggplot(survey_1, aes(x = Prop, y = reorder(Description, Prop), color = rev(factor(Preference)))) +
  geom_point(size = 1.8) +
  facet_grid(Gender~.) +
