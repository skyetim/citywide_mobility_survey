...
## How does gender affect residents' choices of transportation? 
...
The majority of respondents (above 70%), who hold the opinion that bicycling is inexpensive or / and convenient, are those prefer riding a bicycle to get around the city, both females and males. Also, besides these two features, they also think highly of other aspects of bicycle, including safety, speed, etc. On the contrary, for those not prefering bicycling, only a small proportion of them think bicycling has such characteristic, when they were asked. However, figure "Gender Composition" shows that a large percent of males choose bicycle as the mode they would choose to get around the city, and also a large proportion of females would not like to ride a bicycle. The possible interpretation is that what males are concerned about most is different from that of females. Comparing safety with cost as well as comfort level, males value the latter more, and females may be concerned about safety most, which is consistent with our guess.
...
survey_1 <- merge(travel_mode, Survey, by = "UniqueID", all.y = TRUE) %>%
  mutate(Preference = ifelse(Bicycle == 1, "Prefer", "Not Prefer")) %>%
  subset(select = c(1, 10:20)) %>%
  gather(key = "Description", value = "Choice", -c(1:2, 11, 12)) %>%
  filter((Choice == 1) & (Description %in% c("Convenient", "Reliable", 
                                           "Fast", "Inexpensive", 
                                           "Safe", "Comfortable"))) %>%
  subset(select = c(1:5)) %>%
  group_by(Gender, Description, Preference) %>%
  summarise(Num = sum(allwt)) %>%
  ungroup() %>%
...
ggplot(survey_1, aes(x = Prop, y = reorder(Description, Order), color = rev(factor(Preference)))) +
  geom_point(size = 1.8) +
  facet_grid(Gender~.) +
  scale_x_continuous(labels = scales::percent) +
  labs(x = "Proportion", y = "Description", 
       title = "Perception of Bicycling",
       subtitle = "Faceted by residents who do or do not prefer bicycling",
       caption = "Source: https://data.cityofnewyork.us/Transportation/Citywide-Mobility-Survey-Main-Survey/dd6w-hnq9") + 
  scale_color_manual(values = c("gray40", "sienna3"))+
  theme_economist() +
  theme(axis.text.x = element_text(size = 8, angle = 0), 
...
HH::likert(Gender ~ ., 
           ...
           horizontal = FALSE,
           col = c("steelblue3", "steelblue2", "bisque", "orange", "orange3", "orange4"),
           alpha = 0.2) 
```
...
## How does age affect residents' choices of transportation? 
...
## Why do residents use ride hailing services in recent years? 
...
### Factors that influence frequency of using ride hailing services
